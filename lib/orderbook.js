...
class OrderBookSim {
  constructor(symbol, tickSize, rng, maxOrders = 500) {
    this.symbol = symbol;
  ...
  cycle(midPrice) {
    ...
    // Prune oldest orders if book exceeds cap
    if (this.orders.size > this.maxOrders) {
      const ids = [...this.orders.keys()];
      const excess = ids.slice(0, this.orders.size - this.maxOrders);
      for (const id of excess) {
        this.orders.delete(id);
        msgs.push({ type: 'ob', action: 'delete', symbol: this.symbol, id });
      }
    }
